...
func deposit(w http.ResponseWriter, req *http.Request) {
	...
	if number, error := strconv.ParseFloat(numberqs, 64); error != nil {
		fmt.Fprintf(w, "Invalid account number!")
	} else if amount, error := strconv.ParseFloat(amountqs, 64); error != nil {
...
func transfer(w http.ResponseWriter, req *http.Request) {
	...
	if number, err := strconv.ParseFloat(numberqs, 64); err != nil {
		...
		if accountA, ok := accounts[number]; !ok {
			...
			// 注意这里的函数调用方法和参数
			// accountA和accountB的类型均为*CustomAccount
			// accountA在调用Transfer时，实际上是调用的嵌入方法（见“在 Go 中使用方法”一章），也就是其成员*Account的方法
			// 而对于函数Transfer的参数，要传入的是一个*Account对象，所以需要在传入时显式的指定accountB.Account
